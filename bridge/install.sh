...
# --- 1b. generate a launcher that hardcodes the absolute node path -----------
# Chrome launches native-messaging hosts with a minimal PATH, so a
# `#!/usr/bin/env node` shebang fails when node lives under nvm/homebrew (the
# host then dies with "Native host has exited."). The launcher uses /bin/sh
# (always present) and an absolute node path, so it needs nothing on PATH.
LAUNCHER="$HERE/launch-host.sh"
...
cat > "$LAUNCHER" <<EOF
#!/bin/sh
# Machine-local launcher (gitignored) — regenerated by install.sh.
...
# The manifest points at the launcher (not the .js directly) so Chrome's
# minimal-PATH spawn can still find node.
sed -e "s#__BRIDGE_PATH__#$LAUNCHER#" \
