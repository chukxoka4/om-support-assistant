...
export function draftIsRevisitPending(d) {
  ...
  if (d.outcome === "rewrote" && d.correction_logged) return false;
  if (d.outcome === "edited" && d.correction_logged) return false;
  return true;
}
...
export async function setLibraryOverride(library) {
  if (library === null || library === undefined) {
    await chrome.storage.local.remove(KEYS.library);
    return;
  }
