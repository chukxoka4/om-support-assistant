...
const HYPOTHESIS_VERDICTS = ["yes", "partially", "no", "insufficient_data"];
...
export function validateWpsaShape(payload) {
  ...
  const normalised = {
    ...
    // Optional, additive — only present when the user supplied hypotheses
    // upstream. Unknown supported values are dropped to null rather than
    // failing validation, so a flaky analyser can't break the whole report.
    hypothesisFindings: Array.isArray(payload.hypothesisFindings)
      ? payload.hypothesisFindings
          .filter((h) => h && typeof h === "object" && typeof h.hypothesis === "string" && h.hypothesis.trim())
          .map((h) => ({
            hypothesis: h.hypothesis.trim(),
            supported: HYPOTHESIS_VERDICTS.includes(h.supported) ? h.supported : null,
            evidence: typeof h.evidence === "string" ? h.evidence : "",
            supportingTicketIds: Array.isArray(h.supportingTicketIds)
              ? h.supportingTicketIds.filter((x) => typeof x === "string" || typeof x === "number").map(String)
              : [],
            frictionReframe: typeof h.frictionReframe === "string" && h.frictionReframe.trim() ? h.frictionReframe.trim() : null,
            oiImpact: typeof h.oiImpact === "string" && h.oiImpact.trim() ? h.oiImpact.trim() : null,
            recommendedAction: typeof h.recommendedAction === "string" && h.recommendedAction.trim() ? h.recommendedAction.trim() : null
          }))
      : []
  };
