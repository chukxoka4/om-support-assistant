...
import { bar, line, pie, stackedBar, counter, multiLine } from "./charts.js";
...
.oi-verdict { display: inline-block; padding: 2px 7px; border-radius: 4px; font-size: 10px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.4px; margin-left: 6px; vertical-align: middle; }
...
.escalation-verdict { display: inline-block; padding: 4px 10px; border-radius: 4px; font-size: 12px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.6px; margin-bottom: 8px; }
.escalation-verdict.escalate { background: #dcfce7; color: #166534; }
.escalation-verdict.playbook_only { background: #e0e7ff; color: #3730a3; }
.escalation-verdict.watch { background: #fef3c7; color: #92400e; }
.lever-pill { display: inline-block; padding: 2px 8px; border-radius: 10px; font-size: 11px; font-weight: 600; }
.lever-pill.lever-churn { background: #fee2e2; color: #991b1b; }
.lever-pill.lever-reactivations { background: #fef3c7; color: #92400e; }
.lever-pill.lever-upgrades { background: #dcfce7; color: #166534; }
.lever-pill.lever-cost_reduction { background: #e0e7ff; color: #3730a3; }
.lever-pill.lever-none { background: #f3f4f6; color: #6b7280; }
.mve em { color: #4b5563; }
...
.hyp-card { border: 1px solid #e3e5e8; border-radius: 6px; padding: 12px 14px; margin-bottom: 10px; background: #fff; }
.hyp-head { display: flex; align-items: flex-start; gap: 8px; margin-bottom: 4px; }
.hyp-text { font-size: 13px; font-weight: 600; color: #1f2328; flex: 1; }
.hyp-badge { display: inline-block; padding: 2px 8px; border-radius: 10px; font-size: 10px; text-transform: uppercase; letter-spacing: 0.5px; font-weight: 700; flex-shrink: 0; }
.hyp-badge.yes { background: #dcfce7; color: #166534; }
.hyp-badge.partially { background: #fef3c7; color: #92400e; }
.hyp-badge.no { background: #fee2e2; color: #991b1b; }
.hyp-badge.insufficient_data { background: #f3f4f6; color: #4b5563; }
.hyp-evidence { font-size: 12px; color: #4b5563; margin: 4px 0; }
.hyp-tickets { font-size: 11px; color: #6b7280; margin-top: 4px; }
.hyp-mini { margin-top: 8px; padding-top: 8px; border-top: 1px dashed #e5e7eb; }
.hyp-mini summary { cursor: pointer; font-size: 12px; color: #2563eb; user-select: none; outline: none; }
.hyp-mini summary:hover { text-decoration: underline; }
.hyp-mini[open] summary { margin-bottom: 6px; }
.hyp-mini-row { font-size: 12px; color: #1f2328; margin: 6px 0; }
.hyp-mini-row strong { color: #4b5563; font-weight: 600; }
@media print {
...
function header({ weekStart, weekEnd, analysedAgent, reportAuthor, generatedAt }) {
  ...
  const author = (reportAuthor || "").trim();
  const agent = (analysedAgent || "").trim();

  let attribution;
  if (agent && author && agent !== author) {
    attribution = `About ${agent} · prepared by ${author}`;
  } else if (author) {
    attribution = `By ${author}`;
  } else if (agent) {
    attribution = `About ${agent}`;
  } else {
    attribution = "";
  }
  const attribLine = attribution ? ` · ${attribution}` : "";

  return `
    <div class="report-header">
      <div class="report-title">Weekly Support Insights</div>
      <div class="report-meta">${escapeHtml(range)}${escapeHtml(attribLine)}${stamp ? " · " + escapeHtml(stamp) : ""}</div>
    </div>`;
}
...
function sectionAuditMetrics(metrics) {
  if (!metrics) return "";
  // Range-based shape (computeAuditMetricsForRange) carries `counts` and
  // `activitySeries`; legacy shape (computeAuditMetrics) carries
  // `composedThisWeek` + `windowDays` + per-resolution-status `*ThisWeek`
  // fields. Render both — fields that don't apply to one shape collapse.
  const isRange = !!metrics.counts;
  const lib = metrics.library || {};
  const sug = metrics.suggestions || {};
  const ctr = metrics.suggestionCtr || {};
  const ctx = metrics.customerContext || {};

  const addedFootnote = isRange
    ? (lib.addedInRange ? `+${lib.addedInRange} in range` : "no new entries")
    : (lib.addedThisWeek ? `+${lib.addedThisWeek} this week` : "no new entries");

  const sugApplied = isRange ? (sug.appliedInRange || 0) : (sug.appliedThisWeek || 0);
  const sugRejected = isRange ? (sug.rejectedInRange || 0) : (sug.rejectedThisWeek || 0);
  const sugDeferred = isRange ? (sug.deferredInRange || 0) : (sug.deferredThisWeek || 0);
  const sugTotalResolved = isRange ? (sug.totalResolvedInRange || 0) : (sug.totalResolvedThisWeek || 0);

  // Top-line activity counters (range-based only — these are the user's
  // headline asks: split, non-overlapping, bucketed by event timestamp).
  const activityCounters = isRange ? [
    counter({ label: "Drafts generated",
              value: metrics.counts.generated ?? 0,
              footnote: "by generation timestamp" }),
    counter({ label: "Sent / approved",
              value: metrics.counts.sent ?? 0,
              footnote: "by outcome timestamp" }),
    counter({ label: "Manager rewrites",
              value: metrics.counts.rewritten ?? 0,
              footnote: "by outcome timestamp" }),
    counter({ label: "Reached an outcome",
              value: metrics.counts.reachedOutcome ?? 0,
              footnote: `of ${metrics.counts.generated ?? 0} generated this range` })
  ] : [];

  const counters = [
    counter({ label: "Library size", value: lib.total ?? 0, footnote: addedFootnote }),
    counter({ label: "Refined entries", value: lib.rewritesAbsorbedAllTime ?? 0,
              footnote: "rewrites absorbed (all time)" }),
    counter({ label: isRange ? "Suggestions resolved in range" : "Suggestions resolved this week",
              value: sugTotalResolved,
              footnote: `${sugApplied} applied · ${sugRejected} rejected · ${sugDeferred} deferred` }),
    ...
    counter({ label: "Customer-context coverage",
              ...
              footnote: ctx.total > 0 ? `${ctx.withContext}/${ctx.total} replies enriched` : "no compose drafts in range" }),
    counter({ label: "Ready-to-Send rate",
              value: metrics.readyToSend != null ? `${metrics.readyToSend}%` : "—",
              footnote: isRange ? "of drafts that reached an outcome in range" : "personal review pattern" })
  ];

  const librarySeries = (metrics.librarySeries || []).map((p) => ({ x: p.x, y: p.y }));
  const activitySeries = isRange && Array.isArray(metrics.activitySeries) ? metrics.activitySeries : null;
  const sugItems = [
    { label: "Applied", value: sugApplied, color: "#16a34a" },
    { label: "Rejected", value: sugRejected, color: "#dc2626" },
    { label: "Deferred", value: sugDeferred, color: "#6b7280" }
  ];

  const activityChart = activitySeries && activitySeries.length ? `
      <h3>Daily activity in range</h3>
      ${multiLine([
        { name: "Generated", color: "#2563eb",
          points: activitySeries.map((p) => ({ x: p.x, y: p.generated })) },
        { name: "Sent / approved", color: "#16a34a",
          points: activitySeries.map((p) => ({ x: p.x, y: p.sent })) },
        { name: "Manager rewrites", color: "#dc2626",
          points: activitySeries.map((p) => ({ x: p.x, y: p.rewritten })) }
      ])}` : "";

  const librarySeriesHeader = isRange ? "Library size across range" : `Library size — last ${metrics.windowDays || 7} days`;

  return `
    <div class="section">
      <span class="section-tag">Section 2</span>
      <h2 class="section-title">How the AI loop is helping me</h2>
      ${activityCounters.length ? `<div class="counter-row">${activityCounters.join("")}</div>` : ""}
      ${activityChart}
      <div class="counter-row">${counters.join("")}</div>
      <h3>${escapeHtml(librarySeriesHeader)}</h3>
      ${line(librarySeries)}
      <h3>${isRange ? "Suggestions resolved in range" : "Suggestions resolved this week"}</h3>
      ${stackedBar(sugItems)}
    </div>`;
}
...
function leverPill(lever) {
  if (!lever) return "";
  const labels = {
    churn: "Churn", reactivations: "Reactivations", upgrades: "Upgrades",
    cost_reduction: "Cost reduction", none: "No growth lever"
  };
  return `<span class="lever-pill lever-${lever}">${escapeHtml(labels[lever] || lever)}</span>`;
}

function oiCard(oi) {
  if (!oi) return "";
  // Headline is now the escalation verdict — action-oriented.
  const escalation = oi.escalationVerdict || "watch";
  const escalationLabel = {
    playbook_only: "PLAYBOOK ONLY",
    escalate: "ESCALATE",
    watch: "WATCH"
  }[escalation] || escalation.toUpperCase();
  const oldVerdict = oi.verdict
    ? `<span class="oi-verdict ${escapeHtml(oi.verdict)}">${escapeHtml(oi.verdict.toUpperCase())}</span>`
    : "";
  ...
  const lever = oi.primaryGrowthLever
    ? `<div class="oi-row"><strong>Growth lever:</strong> ${leverPill(oi.primaryGrowthLever)}</div>`
    : "";
  const mve = oi.mveBootstrap
    ? `<div class="oi-row mve"><strong>Bootstrap option:</strong> <em>${escapeHtml(oi.mveBootstrap)}</em></div>`
    : "";
  return `
    <div class="oi-card">
      <span class="escalation-verdict escalation-${escalation}">${escapeHtml(escalationLabel)}</span>
      ${oldVerdict}
...
function hypothesisCard(finding) {
  const supported = finding.supported || "insufficient_data";
  ...
  const tickets = (finding.supportingTicketIds || []).length
    ? `<div class="hyp-tickets">Evidence: #${finding.supportingTicketIds.map(escapeHtml).join(", #")}</div>`
    : "";

  // The mini-report (collapsed by default) — only render when the analyser
  // produced any of the impact fields. If all three are null the disclosure
  // would be an empty box; better to omit it entirely.
  const hasMini = finding.frictionReframe || finding.oiImpact || finding.recommendedAction;
  const mini = hasMini ? `
    <details class="hyp-mini">
      <summary>How this reframes leaderboard / O/I — click to expand</summary>
      ${finding.frictionReframe ? `<div class="hyp-mini-row"><strong>Leaderboard reframe:</strong> ${escapeHtml(finding.frictionReframe)}</div>` : ""}
      ${finding.oiImpact ? `<div class="hyp-mini-row"><strong>O/I impact:</strong> ${escapeHtml(finding.oiImpact)}</div>` : ""}
      ${finding.recommendedAction ? `<div class="hyp-mini-row"><strong>Recommended action:</strong> ${escapeHtml(finding.recommendedAction)}</div>` : ""}
    </details>` : "";

  return `
    <div class="hyp-card">
      <div class="hyp-head">
        <div class="hyp-text">${escapeHtml(finding.hypothesis)}</div>
        <span class="hyp-badge ${escapeHtml(supported)}">${escapeHtml(supportedLabel)}</span>
      </div>
      ${finding.evidence ? `<div class="hyp-evidence">${escapeHtml(finding.evidence)}</div>` : ""}
      ${tickets}
      ${mini}
    </div>`;
}

// Combine findings from both scopes, dedupe by hypothesis text. Personal
// findings come first; team findings only add net-new hypotheses (so a
// hypothesis investigated under both scopes doesn't render twice).
function mergeHypothesisFindings(personalWpsa, teamWpsa) {
  const personal = (personalWpsa?.hypothesisFindings) || [];
  const team = (teamWpsa?.hypothesisFindings) || [];
  const seen = new Set(personal.map((h) => h.hypothesis.toLowerCase()));
  const teamNew = team.filter((h) => !seen.has(h.hypothesis.toLowerCase()));
  return [...personal, ...teamNew];
}

function sectionHypotheses(findings) {
  // Hard rule: empty input → render nothing. No header, no shell.
  if (!Array.isArray(findings) || !findings.length) return "";
  return `
    <div class="section">
      <span class="section-tag">Hypotheses</span>
      <h2 class="section-title">Hypotheses investigated (${findings.length})</h2>
      ${findings.map(hypothesisCard).join("")}
    </div>`;
}
...
// Public API.
//   buildReportHtml({ personalWpsa, teamWpsa, audit, ask, reportAuthor })
// All inputs optional except at least one of personalWpsa / teamWpsa / audit.
export function buildReportHtml({ personalWpsa, teamWpsa, audit, ask, reportAuthor } = {}) {
  const meta = teamWpsa?.meta || personalWpsa?.meta || {};
  const headerData = {
    weekStart: meta.weekStart,
    weekEnd: meta.weekEnd,
    analysedAgent: personalWpsa?.meta?.agent || (teamWpsa ? null : meta.agent),
...
<div class="report">
  ...
  ${sectionHypotheses(mergeHypothesisFindings(personalWpsa, teamWpsa))}
  ${sectionAsk(ask)}
</div>
</body></html>`;
}
