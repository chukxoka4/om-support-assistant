...
export async function computeMetrics(windowDays = 30) {
  ...
  const acceptedAny = sentAsIs + managerApproved;
  const readyRate = withOutcome.length ? Math.round((acceptedAny / withOutcome.length) * 100) : null;
  const managerRate = withOutcome.length ? Math.round((managerApproved / withOutcome.length) * 100) : null;
  ...
  return {
    ...
    sentAsIs, managerApproved, edited, rewrote,
