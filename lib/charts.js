...
// All series must share the same x labels; first series defines the x axis.
export function multiLine(series, { width = 520, height = 200, valuePrefix = "" } = {}) {
  if (!series.length || !series[0].points?.length) {
    return "<div class='empty-chart'>no data</div>";
  }
  const padL = 36, padR = 12, padT = 12, padB = 38;
  const innerW = width - padL - padR;
  const innerH = height - padT - padB;
  const xs = series[0].points.map((p) => p.x);
  const allYs = series.flatMap((s) => s.points.map((p) => p.y || 0));
  const max = Math.max(1, ...allYs);
  ...
  const paths = series.map((s, si) => {
    ...
    const d = s.points.map((p, i) => `${i === 0 ? "M" : "L"} ${xAt(i).toFixed(1)},${yAt(p.y || 0).toFixed(1)}`).join(" ");
    const dots = s.points.map((p, i) => `<circle cx="${xAt(i).toFixed(1)}" cy="${yAt(p.y || 0).toFixed(1)}" r="2.5" fill="${color}" />`).join("");
    return `<path d="${d}" fill="none" stroke="${color}" stroke-width="2" />${dots}`;
  }).join("");

  const xLabels = xs.map((label, i) =>
    `<text x="${xAt(i).toFixed(1)}" y="${height - 18}" font-size="10" fill="#6b7280" text-anchor="middle">${escapeText(label)}</text>`
  ).join("");

  const legend = series.map((s, si) => {
    const color = s.color || PALETTE[si % PALETTE.length];
    return `<span class="legend-inline"><span class="legend-swatch" style="background:${color}"></span>${escapeText(s.name)}</span>`;
  }).join(" · ");
