...
// Heuristic timestamp detection for custom_attributes — Intercom stores
// many time-typed custom attributes as raw unix seconds with no schema
// hint. Returns { ms, source } if the value looks like a timestamp, else
// null. Trusts hints in `key` (suffix `_at`, contains `date`/`time`) but
// also recognises numeric values within the plausible unix-time range,
// so unhinted keys still format correctly.
export function detectTimestamp(key, value) {
  if (value == null || value === "") return null;
  const keyLooksTimey = typeof key === "string" &&
    /(_at|_date|_time|expires|signup|signed_up)$/i.test(key);

  if (typeof value === "number" && Number.isFinite(value)) {
    // 10-digit unix-seconds covers 2001–2286.
    // 13-digit unix-ms covers 2001–2286 in ms.
    const seemsSec = value > 9.5e8 && value < 4e10;
    const seemsMs = value > 9.5e11 && value < 4e13;
    if (seemsSec) return { ms: value * 1000, source: "seconds" };
    if (seemsMs) return { ms: value, source: "ms" };
    if (keyLooksTimey) return null; // hint says timey, value out of range
  }

  if (typeof value === "string") {
    if (/^-?\d+$/.test(value.trim())) {
      const n = Number(value);
      if (n > 9.5e11) return { ms: n, source: "ms" };
      if (n > 9.5e8 && n < 4e10) return { ms: n * 1000, source: "seconds" };
      return null;
    }
    // ISO string or any RFC date the platform parses.
    if (keyLooksTimey || /\d{4}-\d{2}-\d{2}/.test(value)) {
      const t = Date.parse(value);
      if (!Number.isNaN(t)) return { ms: t, source: "iso" };
    }
  }
  return null;
}

// Render a timestamp ms value as "<rel> · YYYY-MM-DD" where <rel> is
// "today", "Nd ago", or "in Nd".
export function formatTimestampMs(ms) {
  if (typeof ms !== "number" || !Number.isFinite(ms)) return "";
  const diffDays = Math.round((Date.now() - ms) / (24 * 3600 * 1000));
  let rel;
  if (diffDays === 0) rel = "today";
  else if (diffDays > 0) rel = `${diffDays}d ago`;
  else rel = `in ${Math.abs(diffDays)}d`;
  const d = new Date(ms);
