...
// ---------- equivalence match ----------
// Two entries are equivalent when product + the four dropdowns match AND the
// first 60 chars of scenario_instruction match (normalised: lowercased,
// trimmed, internal whitespace collapsed). Adding the instruction prefix as a
// 6th dimension prevents two genuinely different scenarios that happen to
// share dropdowns from silently collapsing into one (the second instruction
// would have been lost under the old shape).
//
// `scenarioInstruction` is optional. When omitted (legacy callers, or when
// the parser didn't produce one), the function falls back to the original
// 5-dimension match for backward compatibility.
const INSTRUCTION_PREFIX_LEN = 60;

export function normaliseInstructionPrefix(instruction) {
  if (!instruction) return "";
  return instruction
    .toLowerCase()
    .replace(/\s+/g, " ")
    .trim()
    .slice(0, INSTRUCTION_PREFIX_LEN);
}

export async function findEquivalent({ product, dropdowns, scenarioInstruction }) {
  const all = await getAllEntries();
  const incomingPrefix = normaliseInstructionPrefix(scenarioInstruction);
  return (
    all.find((e) => {
      if (e.product !== product) return false;
      if (e.dropdowns.goal !== dropdowns.goal) return false;
      if (e.dropdowns.audience !== dropdowns.audience) return false;
      if (e.dropdowns.tone !== dropdowns.tone) return false;
      if (e.dropdowns.mode !== dropdowns.mode) return false;
      if (!incomingPrefix) return true; // legacy 5-field match
      return normaliseInstructionPrefix(e.scenario_instruction) === incomingPrefix;
    }) || null
  );
}
