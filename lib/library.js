...
// already seen a preview and confirmed; this function makes the mutation real.
//
// Returns:
//   { applied: true, type, change }                 — entry mutated
//   { applied: false, status: "needs_manual" }      — split_entry: surfaced
//                                                      to the UI for manual
//                                                      handoff (no mutation)
//   { applied: false, error: "..." }                — entry / suggestion
//                                                      missing or no change
export async function applySuggestion(entryId, suggestionId) {
  const { [LIBRARY_KEY]: lib = [] } = await chrome.storage.local.get(LIBRARY_KEY);
  const idx = lib.findIndex((e) => e.id === entryId);
  if (idx === -1) return { applied: false, error: "entry not found" };
  const entry = lib[idx];
  const queue = entry.pending_suggestions || [];
  const sIdx = queue.findIndex((s) => s.id === suggestionId);
  if (sIdx === -1) return { applied: false, error: "suggestion not found" };
  const suggestion = queue[sIdx];
  const change = (suggestion.ai_analysis?.proposed_changes || [])[0];
  if (!change) return { applied: false, error: "no proposed change" };
  ...
  if (change.type === "refine_instruction") {
    ...
    await saveAll(lib);
    return { applied: true, type: change.type, change };
  }
  ...
  if (change.type === "split_entry") {
    // Risky — requires a new entry. Surface to the UI for manual creation.
    queue[sIdx].status = "needs_manual";
    queue[sIdx].resolved_at = now;
    await saveAll(lib);
    return { applied: false, status: "needs_manual", change };
  }

  return { applied: false, error: `unknown change type: ${change.type}` };
}
