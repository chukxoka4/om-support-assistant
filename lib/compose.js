...
export async function compose({
  ...
  const system = await buildSystemPrompt({
    ...
    includeKbReasoning: true
  });
  const user = buildUserPrompt({ draft, promptExtra, searchResults, productDoc, customerContext });

  // "reason" lets the connector search the local KB while drafting (DEC-F).
  // HTTP providers ignore mode; claude-code with no KB silently runs transform.
  const { text, error, provider: usedProvider } = await callLLM({ provider, system, user, mode: "reason" });
