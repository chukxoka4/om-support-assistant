...
function buildUserPrompt({ draft, promptExtra, searchResults, productDoc }) {
  const parts = [];
...
export function parseStructuredOutput(raw) {
  if (!raw) return { reason: "", versionA: "", versionB: "", cleanPrompt: "", scenarioSummary: "", raw: "" };
  const text = raw.replace(/\r\n/g, "\n");
  const grab = (re) => (text.match(re)?.[1] || "").trim();
  return {
    reason: grab(/REASON:\s*([\s\S]*?)(?=\n\s*VERSION A|$)/i),
    versionA: grab(/VERSION A[^\n:]*:\s*([\s\S]*?)(?=\n\s*VERSION B|$)/i),
    versionB: grab(/VERSION B[^\n:]*:\s*([\s\S]*?)(?=\n\s*CLEAN_PROMPT|$)/i),
    cleanPrompt: grab(/CLEAN_PROMPT:\s*([\s\S]*?)(?=\n\s*SCENARIO_SUMMARY|$)/i),
    scenarioSummary: grab(/SCENARIO_SUMMARY:\s*([\s\S]*?)$/i),
    raw: text
  };
}
...
export async function compose({
  ...
  conversationId = null, libraryEntryId = null
}) {
  ...
  const user = buildUserPrompt({ draft, promptExtra, searchResults, productDoc });
  ...
  if (libraryEntryId) {
    ...
    const existing = await findEquivalent({ product, dropdowns });
    if (existing) {
  ...
  return { parsed, raw: text, draftId: record.id, provider: usedProvider, libraryEntryId: libraryId };
}
