...
- **KB reasoning (connector only, DEC-F/D39):** compose runs in `reason` mode, so when the `claude-code` connector serves the call it searches the local support-desk knowledge base (`patterns/INDEX.md` → matching pattern sections) *while drafting* and cites the verified doc URL. The KB instruction is conditional — HTTP providers and a KB-less machine get byte-identical plain-LLM behavior. The KB is strictly read-only (DEC-G/D40): reason mode's tool allowlist is `Read,Grep,Glob`, so no ticket text can be written to it.
